...
lot of file_get_contents() and file_put_contents() calls to store and
fetch this information. This is quite a lot of overhead for loading PHP
classes. Even without a lot of class aliases (e.g. in CMS7) this
...
installation differently, including e.g. Unit and Functional Tests
...
package created by Helmut Hummel (available on
https://github.com/helhum/class-alias-loader) which serves as a facade
...
variable called "TYPO3_COMPOSER_AUTOLOAD" which needs to be set on
server-side level.
...
the same logic as before. Please note that during the CMS7 development
this might be improved further.

See the following reviews for the change:

-  https://review.typo3.org/#/c/36091/
-  https://review.typo3.org/#/c/36076/
-  https://review.typo3.org/#/c/36644/

Backport Considerations
=======================

We backported the base functionality to TYPO3 CMS 6.2. The integration
of the Composer class loader decreases the loading of a frontend page
inside a real-life installation up to 10% on a “first hit” (no caches
set up) and up to 8% on a fully cached page. This leads to fewer race
conditions while setting up class loader caches, as the TYPO3 class
loader cache is not as populated as much as before. The IO access to the
file systems is decreased due to less lookups of classes in temporary
files. If used in a pure-composer-setup together with the environment
variable "TYPO3_COMPOSER_AUTOLOAD" activated, the performance increase
will be even bigger.

The integration is a 1:1 backport without adaptations to the base code,
only to load different class maps. The functionality contains proper
fallbacks, so there will be no breaking changes.

Do I need a Packages/ directory then for 6.2 as well?
-----------------------------------------------------

No. For 6.2 a pre-generated autoload.php (and additional necessary
files) are committed to “typo3/contrib/vendor” directory and used for
the TYPO3 core if it is not installed via composer.
...
We used xhprof to profile the overall performance with a default caching
setup shipped with the TYPO3 CMS default configuration.

We measured non-cached page (after clearing all caches, including system
caches) and a fully cached page. Of course any TYPO3 backend requests
profit from this change as well.

We noticed a decrease in request times, and the number of function
calls, as well as large drop of file_get_contents() and
file_put_contents() in the function calls.

We measured these changes with a 6.2 installation as well as a TYPO3
installation on master, the first one set up with the tarball
distribution shipped on typo3.org.
